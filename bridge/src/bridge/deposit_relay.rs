...
use futures::{self, Future, Stream, Poll, future};
use web3::Transport;
		...
		loop {
			let next_state = match self.state {
				DepositRelayState::Wait => {
					...
					let balance_required = U256::from(self.app.config.txs.deposit_relay.gas) * U256::from(self.app.config.txs.deposit_relay.gas_price) * U256::from(len);
